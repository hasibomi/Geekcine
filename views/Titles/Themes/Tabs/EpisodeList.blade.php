...
<div class="text-center">
    <a href='{{ url(Str::slug(trans("main.series")) . "/" . $data->getId() . "/seasons/$num/episodes/create") }}' class="btn btn-primary">
        <i class="fa fa-video-camera visible-xs"></i>
        <strong class="hidden-xs">{{ trans('main.create new epi') }}</strong>
    </a>
</div>
@endif
...
@foreach($data->getSeasons($num)->episode as $k => $v)
	<div class="row">
        <div class="col-sm-5">
            <div class="item">
                <div class="pos-rlt">
                    <img src="{{{ $v->poster ? asset($v->poster) : asset($data->getPoster()) }}}" alt="{{ 'Poster of ' . $v->title }}" class="img-full">
                </div>
                <div class="padder-v text-center">
                    {{ trans('main.release date') }}: {{{ $v->release_date }}}
                </div>
            </div>
         </div>
         
         <div class="col-sm-7">
            <span class="font-thin h3 text-white">{{ trans('main.episode') }} {{{ $v->episode_number }}} - {{ $v->title }}</span>
            ...
            @if (Helpers::hasAccess('titles.delete'))
                {{ Form::open(array('url' => Str::slug(trans('main.series')) . '/' . $data->getId() . "/seasons/$num/episodes/{$v->id}", 'method' => 'delete', 'class' => 'delete-form')) }}
                    <button type="submit" title="{{ trans('main.delete') }}" class="btn btn-info"><i class="fa fa-trash-o"></i> {{ trans('main.delete ep') }}</button>
